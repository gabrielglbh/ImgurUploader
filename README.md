...
1. Añadir paginación al GET de imágenes de Imgur. Hay un endpoint que te permite añadir paginación, pero no describe cual es límite de cada página. Para añadir la paginación haría falta añadir un listener al scroll del RecyclerView para saber cuando se ha llegado al final de la lista y, ahí, pedir más imágenes aumentando la página en 1.

2. Cambiar SharedPreferences por algun otro almacenaje seguro para guardar el `accessToken` y el `refreshToken`, como por ejemplo: [EncryptedSharedPrefences](https://developer.android.com/reference/androidx/security/crypto/EncryptedSharedPreferences).

3. Añadir un método más seguro y encriptado para guardar siempre las credenciales de `CLIENT_ID` y `CLIENT_SECRET_ID` en el propio proyecto, como por ejemplo en la BuildConfig o en variables de entorno.
